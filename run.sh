...
config_file='/usr/local/docpath/activespoolerpack/activespooler/Configuration/dpactivespooler.ini'
db_type="${DB_TYPE:-MySQL}"
db_host="${DB_HOST:-localhost}"
db_port="${DB_PORT:-3306}"
db_user="${DB_USER:-root}"
db_pass="${DB_PASS:-root}"
db_name="${DB_NAME:-activespooler}"
aim_url="${AIM_URL}"
aim_token="${AIM_TOKEN}"
...
echo "log file name = dpactivespooler.log" >> "$config_file"
...
echo "storage directory = cache" >> "$config_file"
...
cd /usr/local/docpath/activespoolerpack/activespooler/Bin
if [[ -n $transaction_id ]]
then
    exec java -jar dpactivespooler.war -logmodeCONSOLE -licensetransactionid$transaction_id -licenseaddr$license_address -licenseport$license_port -licenseshutdownsession$shutdown_session
else 
    exec java -jar dpactivespooler.war -logmodeCONSOLE
fi